...
def display_video_player(video: Dict):
    ...
    if video_path:
        if video_path.startswith(('http://', 'https://')):
            proxy_url = f"http://10.66.8.56:30012/vision-analyze/video/proxy/{video_path}"
            st.video(proxy_url)
        else:
            try:
                        ...
                        if category == "四、道路环境":
                            ...
                            for sub_tag in sub_items:
                                ...
                                if clean_sub_tag not in current_tags:
                                    st.markdown('<div style="margin: 10px 0; padding-left: 20px;">',
...
def export_results():
    ...
    evaluation_report = {
        ...
        "准确率分析": {
            "标签准确率": f"{(results['correct_tags'] / results['total_tags'] * 100):.2f}%" if results[
                                                                                                   'total_tags'] > 0 else "0%",
            "错打率": f"{(results['wrong_tags'] / results['total_tags'] * 100):.2f}%" if results[
                                                                                             'total_tags'] > 0 else "0%",
            "平均每视频错误标签数": f"{(results['wrong_tags'] / results['processed_videos']):.2f}" if results[
                                                                                                          'processed_videos'] > 0 else "0",
            "平均每视频遗漏标签数": f"{(results['missed_tags'] / results['processed_videos']):.2f}" if results[
                                                                                                           'processed_videos'] > 0 else "0"
        },
...
def show_visualization():
    ...
    # 显示准确率
    total_tags = report['total_statistics']['total_tags']
    if total_tags > 0:
        accuracy = (report['total_statistics']['correct_tags'] / total_tags * 100)
        st.metric("总体准确率", f"{accuracy:.1f}%")
    else:
        st.metric("总体准确率", "暂无数据")
    ...
    with tab2:
        try:
            ...
            with col2:
                accuracy_data = {
                    "overall_accuracy": f"{(report['total_statistics']['correct_tags'] / report['total_statistics']['total_tags'] * 100):.1f}%" if
                    report['total_statistics']['total_tags'] > 0 else "暂无数据",
    ...
    with tab3:
        try:
            ...
            # JSON数据下载按钮
            with col2:
                total_videos = report['total_statistics']['total_videos']
                recall_data = {
                    ...
                    "missed_tags_analysis": {
                        ...
                        "average_missed_per_video": report['total_statistics'][
                                                        'missed_tags'] / total_videos if total_videos > 0 else 0
                    }
                }
                st.download_button(
                    label="下载召回率分析数据",
